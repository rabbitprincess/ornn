...
type Queries struct {
	schema  *Schema
	Custom  []*Query            `hcl:"custom"`  // user defined - not overwritten by schema
	Default map[string][]*Query `hcl:"default"` // auto generated by schema
}
...
type Query struct {
	...
	Comment string  `hcl:"comment,omitempty"`
	Sql     string  `hcl:"sql"`
	Schema  *Schema `hcl:"-"` // 쿼리 제작을 위한 전체 스키마 정보

	// options
	SelectFieldTypes []*SelectFieldType `hcl:"fields,omitempty"`
	InsertMulti      bool               `hcl:"insert_multi,omitempty"`
	UpdateNullIgnore bool               `hcl:"update_null_ignore,omitempty"`
	ErrQuery         string             `hcl:"-"`
	ErrParser        string             `hcl:"-"`
}

// select 만 field type이 있는 이유
// select query 는 bp.hcl 의 schema type 을 통해 타입을 지정할 수 없기 때문에
// 직접 쿼리를 select 를 하고 결과를 추출해 타입에 넣음
// snum, uint 등의 custom type 은 여기서 처리
type SelectFieldType struct {
	Name    string `hcl:"name"`
	TypeGen string `hcl:"type"`
}
